...
## Arquitectura de Directorios para Múltiples IAs

Para que cualquier herramienta de IA (Claude, Gemini, Cursor, Windsurf, etc.) pueda interpretar este proyecto sin fricción, se ha estandarizado la siguiente topología de carpetas en la raíz:

- `docs/`: Documentación técnica (este archivo), resoluciones (`troubleshooting.md`), paginación (`page-numbering.md`), y el contexto universal de AIQ/Rommel Ayala (`context.md`).
- `docs/tasks/`: Tareas, PRDs y planes pendientes por implementar.
- `.cursorrules`: Archivo pivot agnóstico global. Le indica a la IA que inyecte de inmediato en su memoria el contexto desde `docs/context.md` y las reglas desde `AGENTS.md`.
- `AGENTS.md`: Reglas maestras del proyecto, convención sobre metadatos e instrucciones de codificación (Fuente de la verdad técnica).
